# Frontend Mentor - Age calculator app

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=html,js,tailwind" />
  </a>
</p>

<p align="center">
  <img src="assets/images/readme.png" alt="readme" width="80%">
</p>

## User stories

Your users should be able to:

-   [x] View an age in years, months, and days after submitting a valid date through the form
-   [x] Receive validation errors if:
    -   [x] Any field is empty when the form is submitted
    <p align="center">
        <img src="assets/images/emptyField.png" alt="readme - empty fields" width="50%">
    </p>
    -   [x] The day number is not between 1-31
    <p align="center">
            <img src="assets/images/dayInvalid.png" alt="readme - empty fields" width="50%">
        </p>
    -   [x] The month number is not between 1-12
    <p align="center">
            <img src="assets/images/monthInvalid.png" alt="readme - empty fields" width="50%">
        </p>
    -   [x] The date is in the future
    <p align="center">
        <img src="assets/images/yearInvalid.png" alt="readme - error validity" width="50%">
    </p>
    -   The date is invalid e.g. 31/04/1991 (there are 30 days in April)
-   [x] View the optimal layout for the interface depending on their device's screen size
-   [x] See hover and focus states for all interactive elements on the page
